	...
	switch 'array.x.y'
		case 'array.x.y' % plot cell locations and types
			...
			switch m.p.ecc % for Figure 9
				...
				case 10
					%	m.p.wid = .5; % for Figure 1
					%	m.p.wid = .2; % for Figure 10
				case 30, m.p.wid = 1;
		...
		case 'rad.ecc' % calculate radius from spatial frequency response
			...
			m.tasks = 'load plot set export'; % plot it
			m.x = 'ecc'; m.y = 'radius'; m.z = 'resp';
		...
		case 'resp.freqS' % plot spatial frequency tuning
			switch 'no' % add interpolated fit?
				...
				case 'no'
					m.tasks = 'resp reduce fund doMax desc plot set';
		...
		case 'resp.freqS fix' % plot spatial frequency tuning with fixed directions
			...
			%	m.resp.back = [0; 1]; % centre and g.c.
			m.resp.stage = {'gangOff'};
			%	m.resp.back = 0; m.resp.stage = 'back'; % surround
			m.reduce.loc = [0, 0];
			m.fund.prop = 'amp'; % amp, complex, phase
		...
		case 'resp.t' % plot model time course for drifting grating
			...
			m.p.horExp = 0;
			m.resp.stage = {'cone', 'hor', 'back', 'biOff', 'biOn'};
			m.p.pot = 0; m.resp.stage = 'gangOff';
			%	m.p.dir = -126; m.p.freqS = 4.8; m.p.cont = m.p.contMag * [1, -1, 0];
			ana = ''; % '', cont
			switch ana
				case 'cont'
					m.resp.cont = [.3; .5] .* [1, 1, 1];
					m.reduce.group = 'cont'; m.unpack.group = 'cont';
					m.prep.group = 'cont';
			end
			m.reduce.loc = [0, 0]; % m.reduce.loc = [-.1, -.07];
			...
			m.plot.group = 'loc';
			m.plot.line = {'stage'};
			switch ana, case 'cont', m.plot.line = {'stage', 'cont'}; end
...
function m = setDef(m) % set default metadata
	...
	m.p.file = ''; % data file
	m.p.horExp = 0; % 1: horizontal cells have exponential receptive fields
