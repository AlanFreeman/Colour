...
function dpot = calDeriv(t, pot, m) % calc. derivatives of model equations
	...
	j = 0; % index for potential
	for i = ["Off", "On"] % loop over centre signs
		w = "wBiGang" + i; bi = "bi" + i; gang = "gang" + i; j = j + 1;
			% construct names
		dp.(gang) = rate * (m.p.(w) * p.(bi) + m.p.potRest(j) - p.(gang));
...
function m = calStruct(m) % calculate the model structure
	...
	m.p.radBack = 10 ^ (c(1) + c(2) * normcdf(e, c(3), c(4))); % f'back c.f. (deg)
	...
	if m.p.horExp % horizontal cells have exponential receptive fields
		m.p.radSur = sqrt(m.p.radOpt ^ 2 + m.p.radBack ^ 2); % sur. rad. (deg)
	else
		m.p.radSur = sqrt(m.p.radOpt ^ 2 + 2 * m.p.radHor ^ 2); % sur. rad. (deg)
	end
...
function m = gang(m) % calculate the ganglion cell array
	...
	%	Set resting generator potential
	m.p.potRest = m.p.actRest / m.p.kRect; % resting impulse rate (mV): [off, on]
...
function p = solveF(m) % solve the model equations in the frequency domain
	...
	%	Calculate surround convergence function
	if m.p.horExp % horizontal cells have exponential receptive fields
		sur = m.p.kSur * m.p.wConeBack * aT; % cone to feedback signal: ls x ls
	else % horizontal cells assumed to have Gaussian receptive fields
		sur = back * h; % cone to horizontal, then horizontal to feedback: ls x ls
	end
	...
	pC = (eye(ls) + m.p.back * c * sur) \ (c * d); % cone resp. (mV): ls x 1
	...
	if m.p.horExp % horizontal cells have exponential receptive fields
		p.back = m.p.kSur * m.p.wConeBack * (aT .* p.cone);
	else
	...
	for i = ["Off", "On"] % loop over centre signs
		...
		p.(gang)(:, 1) = fs * m.p.potRest(j); % resting potential (mV): ls x fs
	end
...
function p = solveT(m) % solve the model equations in the time domain
	...
	pRest = m.p.potRest; % ganglion cell resting potential (mV): [off, on]
	p.gangOff(:) = pRest(1); p.gangOn(:) = pRest(2); % ganglion cells
...
function m = weight(m) % calculate convergence function synaptic weights
	...
	%	Calculate cone to feedback attenuation when h.c. have exponential r.f.
	if m.p.horExp % horizontal cells have exponential receptive fields
		r = m.p.radBack; % feedback convergence function radius (deg): 1 x 1
		w = calConv(locI, locO, r); % distance-based attenuation: cs x cs
		w(:, i) = 0; % H1 hor. cells receive little input from S-cones: cs x cs
		w = w ./ sum(w, 2); % sum of input weights is 1: cs x cs
		m.p.wConeBack = w; % cone to horizontal cell synaptic weights: cs x cs
	end
